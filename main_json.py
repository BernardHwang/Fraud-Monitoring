...
# Email credentials
SMTP_SERVER = '192.168.88.161'
SMTP_PORT = 25
EMAIL_ADDRESS = 'AIOpsMBSB@coreconsulting.asia'
TO_EMAIL = 'bernard.hwang@coreconsulting.asia'
PLATFORM = 'CIB'
...
if not os.path.exists(today_individuals_csv):
  summary_data = [
    ['Time','Sender Name','Sender Acc No','Beneficiary Name','Payment Type','Transfer Amount','Status','Platform']
  ]
  print(f"{today_date}.csv not found. Creating a new file.")
...
def send_email(body):
  ...
  msg['Subject'] = f'[AUTOMATION] - [DTB][CIB] - Criteria {today_date} Violated - {today_date}'
...
def summary_email(): 
  msg = MIMEMultipart("alternative")
  msg['Subject'] = f'[AUTOMATION]-[DTB][CIB] - Monetary Transaction Summary {today_date} '
  msg['From'] = EMAIL_ADDRESS
  msg['To'] = TO_EMAIL

  summary_data = [
    ["DTB/CIB Monetary Transaction Criteria Summary"],
  ...
  for index, row in df.iterrows():
    ...
    data = [ 
      ...
      row['Occurrence 002'] // 3 if row['Occurrence 002'] >= 3 else 0,
      round(row['Occurrence 003'] / 10) * 10 if row['Occurrence 003'] >= 10 else 0,
      row['Occurrence 003'] // 10 if row['Occurrence 003'] >= 10 else 0,
      row['Occurrence 004'], 
      row['Occurrence 004'],
      round(row['Occurrence 005'] / 3) * 3 if row['Occurrence 005'] >= 3 else 0,
      row['Occurrence 005'] // 3 if row['Occurrence 005'] >= 3 else 0
    ]
  ...
  with open(yesterday_csv, 'rb') as f:
    ...
    part.add_header(
      'Content-Disposition',
      f'attachment; filename="{yesterday_date}_DTB CIB_Monetary Transaction Summary.csv"',
    )
    msg.attach(part)
  ...
  with open(yesterday_individuals_csv, 'rb') as f:
    ...
    part.add_header(
      'Content-Disposition',
      f'attachment; filename="{yesterday_date}_DTB CIB_Monetary Individuals Transactions.csv"',
    )
    msg.attach(part)
...
def monitoring():
  ...
  # api_url = "https://apm.mbsb.com.my/e/ccc7c14a-9448-4122-90bb-fd8b2cb1563e/api/v2/metrics/query?metricSelector=calc%3Aservice._cib_fraudmonitoring&from=-2m&to=-2m"
    ...
    for result in results:
      ...
      for data in datas:
        ...
        with open(today_individuals_csv, mode='a', newline='') as file:
          writer = csv.writer(file)
          writer.writerow([time, dimension_sender, str(dimension_sender_id), dimension_beneficiary_name, dimension_transfer_type, dimension_amount, 'Completed', PLATFORM])
